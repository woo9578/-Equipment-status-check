...
function formatDuration(time) {
  let duration = moment.duration(moment().diff(moment().format(time)));
...
function Home() {
  const smartcastKds = [
    {num: 161, ip:"192.168.0.161", version: "v 2.93", status: true, time:"2023-10-10 03:56:43", usetime:""},
    {num: 162, ip:"192.168.0.162", version: "v 2.93", status: true, time:"2023-10-11 03:56:33", usetime:""},
    {num: 163, ip:"192.168.0.163", version: "v 2.93", status: true, time:"2023-10-12 03:56:23", usetime:""},
    {num: 164, ip:"192.168.0.164", version: "v 2.93", status: true, time:"2023-10-13 03:56:53", usetime:""},
    {num: 165, ip:"192.168.0.165", version: "v 2.93", status: true, time:"2023-10-14 03:56:13", usetime:""}
  ]

  const pospc = [
      {name:"배달PC", ip:"192.168.0.199", version:"v 1.01", status: true, time:"05:06:07"},
      {name:"메인포스", ip:"192.168.0.199", version:"v 1.01", status: true, time:"05:06:07"}
  ]

  const server = {name: "KDS SERVER", ip:"192.168.0.189", version: "v 2.9", status: true, time:"2023-10-16 13:05:06"}

  const dids = [
    {num: "1",ip: "192.168.0.199",version: "v 2.93",status: true,time: "05:06:07"},
    {num: "2",ip: "192.168.0.199",version: "v 2.93",status: true,time: "05:06:07"},
  ];

  const kiosks = [
      {num: 1, ip:"192.168.0.161", version: "v 1.01", status: true, time:"03.56.13"},
      {num: 2, ip:"192.168.0.162", version: "v 1.01", status: true, time:"03.56.13"},
      {num: 3, ip:"192.168.0.163", version: "v 1.01", status: false, time:"03.56.13"},
      {num: 4, ip:"192.168.0.164", version: "v 1.01", status: false, time:"03.56.13"},
      {num: 5, ip:"192.168.0.165", version: "v 1.01", status: true, time:"03.56.13"}
  ]

  const difnow = moment.duration(moment().diff(moment().format("2023-10-13 15:30:30")));
  const [currentTime, setCurrentTime] = useState(formatDuration(server.time));
  // useInterval(() => {
  //   setCurrentTime(formatDuration);
  // }, 1000);

  const [updateKds, setUpdateKds] = useState(smartcastKds);

  // useInterval(()=>{
  //     const updateSmartcastKds = updateKds.map((kds) => {
  //       return { ...kds, usetime: formatDuration(kds.time) };
  //     });

  //     setUpdateKds(updateSmartcastKds);
  // },1000)
